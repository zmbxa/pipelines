### This snakemake file can process paired-end bulk RNAseq data (human) ###
...
### Usage:  snakemake --cores 4 -s ~/pipelines/RNAseq/mapping/bulk_PE_RNA_hg38.reminder.smk --use-conda --conda-frontend conda #--dryrun
...
rule star_align:
    ...
    shell:
        "STAR --runThreadN 8 "
        "--genomeDir /storage/zhangyanxiaoLab/share/STAR_index/hg38 "
...
rule featureCounts:
    input:
        ...
        gtf="/storage/zhangyanxiaoLab/xiongxiong/Reference/Annotation/hg38.gencode.annotation.gtf"
    output:
...
rule star_align_te:
    input:
        ...
        r2_clean="fastq/{sample}/{sample}_R2.clean.fastq.gz"
    output:
        bam=temp("te/{sample}/{sample}.Aligned.sortedByCoord.out.bam"),
        temp=temp("te/{sample}/{sample}.Aligned.toTranscriptome.out.bam")
    shell:
        "STAR --runThreadN 8 "
        "--genomeDir /storage/zhangyanxiaoLab/share/STAR_index/hg38  "
        "--readFilesIn {input.r1_clean} {input.r2_clean} "
...
rule tetranscripts:
    ...
    shell:
        ...
        --GTF /storage/zhangyanxiaoLab/xiongxiong/Reference/Annotation/hg38.gencode.annotation.gtf \
        --TE /storage/zhangyanxiaoLab/niuyuxiao/annotations/hg38_rmsk_TE.gtf \
        -b {input}  
        '''
