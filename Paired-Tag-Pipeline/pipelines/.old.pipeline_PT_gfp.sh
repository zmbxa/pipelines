	...
	for prefix in `ls ./fastq/*_R1.fastq.gz | sed "s/_R1.fastq.gz//g; s/\.\/fastq\///g" | sort --parallel ${nthread} -u `; do
		...
		case ${library} in
			"DNA")
				...
				bowtie2 -p ${nthread} -x /storage/zhangyanxiaoLab/niuyuxiao/annotations/bowtie2_index/mm10_gfp -U ${prefix}_FullyLigated_trimmed_R1.fastq.gz | samtools view -bh -@ ${nthread} > tmp.${prefix}_FullyLigated.bam
			...
			"RNA")
				...
				STAR --outSAMunmapped Within --runThreadN ${nthread} --genomeDir /storage/zhangyanxiaoLab/niuyuxiao/annotations/STAR_index/mm10_gfp --readFilesIn ${prefix}_FullyLigated_trimmed_R1.fastq.gz --readFilesCommand zcat --outFileNamePrefix tmp.FullyLigated\. --outSAMtype BAM Unsorted
