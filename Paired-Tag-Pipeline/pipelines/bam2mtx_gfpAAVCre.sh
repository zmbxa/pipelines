...
if [[ -z ${prefix} || -z ${reference_genome} || -z ${input} || -z ${library} ]]; then
	...
	case ${library} in
		"DNA")
			awk '{print $1, $2, $3, $1"-"$2"-"$3, "bin"NR}' OFS='\t' /storage/zhangyanxiaoLab/niuyuxiao/annotations/refBed/binGenome/mm10_gfpAAVCre_bin_5k.bed | bedtools intersect -sorted -F 0.5 -wa -wb -a - -b tmp.UsefulReads.Sorted.bed | cut -f 4,5,9,10 > tmp.Count.txt
