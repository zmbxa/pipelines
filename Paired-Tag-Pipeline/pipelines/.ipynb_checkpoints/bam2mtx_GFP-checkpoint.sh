...
if [[ -z ${prefix} || -z ${reference_genome} || -z ${input} || -z ${library} ]]; then
	...
	if [[ -z ${subMtx} || ${subMtx} = "False" ]]; then
		bedtools bamtobed -i ${input} | awk '{print $1, $2, $3, substr($4,length($4)-24,14), $4}' OFS='\t' > tmp.UsefulReads.bed
	...
	case ${library} in
		"DNA")
			awk '{print $1, $2, $3, $1"-"$2"-"$3, "bin"NR}' OFS='\t' /storage/zhangyanxiaoLab/xiongxiong/Reference/refBed/binGenome/${reference_genome}_bin_5k.bed | bedtools intersect -sorted -F 0.5 -wa -wb -a - -b tmp.UsefulReads.Sorted.bed | cut -f 4,5,9,10 > tmp.Count.txt
			sort --parallel ${nthread} tmp.Count.txt | bedtools groupby -i - -g 1,2,3 -c 4 -o count > ${prefix}_CountMatrix.txt
			;;
		"RNA")
			featureCounts -T ${nthread} -a /storage/zhangyanxiaoLab/qihongjian/datasets/mm10_gfp/genes.gtf -t gene -f -g gene_id --extraAttributes gene_name -M -o tmp.Counts ${input}
