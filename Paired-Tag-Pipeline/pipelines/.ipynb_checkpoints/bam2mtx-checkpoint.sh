...
if [[ -z ${prefix} || -z ${reference_genome} || -z ${input} || -z ${library} ]]; then
	...
	#bedtools bamtobed -i ${input} | awk '{split($4,a,":"); barcode=""; for(i=8;i<=length(a)-1;i++){barcode=barcode":"a[i]}; barcode=substr(barcode,2); print $1, $2, $3, barcode, $4}' OFS='\t' > tmp.UsefulReads.bed
    # this is for zhang CR or chenxu NM
    bedtools bamtobed -i ${input} | awk '{split($4,a,":"); barcode=""; for(i=2;i<=length(a)-1;i++){barcode=barcode":"a[i]}; barcode=substr(barcode,2); print $1, $2, $3, barcode, $4}' OFS='\t' > tmp.UsefulReads.bed
	...
	case ${library} in
		"DNA")
            if [[ ${reference_genome} == "mm10" ]]; then
                reference_genome=/storage/zhangyanxiaoLab/xiongxiong/Reference/refBed/binGenome/mm10_bin_5k.bed
                echo $reference_genome
            fi
        
			awk '{print $1, $2, $3, $1"-"$2"-"$3, "bin"NR}' OFS='\t' $reference_genome | bedtools intersect -sorted -F 0.5 -wa -wb -a - -b tmp.UsefulReads.Sorted.bed | cut -f 4,5,9,10 > tmp.Count.txt
        
			sort --parallel ${nthread} tmp.Count.txt | bedtools groupby -i - -g 1,2,3 -c 4 -o count > ${prefix}_CountMatrix.txt
       
			;;
		"RNA")
			featureCounts -T ${nthread} -a /storage/zhangyanxiaoLab/xiongxiong/Reference/Annotation/${reference_genome}.gencode.annotation.gtf -t gene -f -g gene_id --extraAttributes gene_name -M -o tmp.Counts ${input}
			awk 'NR>3' tmp.Counts | awk '{gsub(/\.[0-9]*/, "", $1); print $2, $3-1, $4, $1, $8, $5, $7}' OFS='\t' | awk '$5>0' > tmp.ExpressedGenes.bed
